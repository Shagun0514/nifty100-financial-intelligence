...
def net_profit_margin(net_profit: float, sales: float) -> Optional[float]:
    """net_profit / sales * 100. None if sales == 0 or either value is missing."""
...
def return_on_equity(net_profit: float, equity_capital: float, reserves: float) -> Optional[float]:
    """ROE = net_profit / (equity + reserves) * 100. None if equity+reserves <= 0 or net_profit missing."""
...
def return_on_assets(net_profit: float, total_assets: float) -> Optional[float]:
    """ROA = net_profit / total_assets * 100. None if total_assets == 0 or net_profit missing."""
...
def asset_turnover(sales: float, total_assets: float) -> Optional[float]:
    """Asset Turnover = sales / total_assets. None if total_assets == 0 or sales missing."""
